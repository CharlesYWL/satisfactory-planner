...
describe('并行产线（方案B）：车道数与机器均分', () => {
  it('lanesForFlow：按最高档带速(1200)算并行满档带数', () => {
    expect(lanesForFlow(0)).toBe(1);
    expect(lanesForFlow(1200)).toBe(1);
    expect(lanesForFlow(1201)).toBe(2);
    expect(lanesForFlow(2400)).toBe(2);
    expect(lanesForFlow(2401)).toBe(3);
  });

  it('laneMachineCount：N 台均分到 L 条，余数摊给靠前车道，合计守恒', () => {
    // 8 台 / 2 条 → 4,4
    expect([0, 1].map((k) => laneMachineCount(8, 2, k))).toEqual([4, 4]);
    // 5 台 / 2 条 → 3,2
    expect([0, 1].map((k) => laneMachineCount(5, 2, k))).toEqual([3, 2]);
    // 1 台 / 2 条 → 1,0（小组分不满所有车道）
    expect([0, 1].map((k) => laneMachineCount(1, 2, k))).toEqual([1, 0]);
    // 7 台 / 3 条 → 3,2,2，合计 = 7
    const split = [0, 1, 2].map((k) => laneMachineCount(7, 3, k));
    expect(split).toEqual([3, 2, 2]);
    expect(split.reduce((a, b) => a + b, 0)).toBe(7);
    // 越界返回 0
    expect(laneMachineCount(8, 2, 2)).toBe(0);
  });

  it('高吞吐（电线 2400）→ laneCount=2，每条 ≤ 单条最高档', () => {
    // 反向 自动路线 50 → 电线 2400（8 台 → 80 台）。
    const { r, machineCountOf, rateOf } = reverseLookups(AUTOMATED_WIRING, 50);
    const plan = computeBlueprint(r.tree, machineCountOf, rateOf);
    const wire = plan.groups.find((g) => g.itemId === WIRE)!;
    expect(wire.totalRate).toBeCloseTo(2400, 6);
    // 全图最粗的带是电线主干 2400 → 需 2 条并行满档带。
    expect(plan.laneCount).toBe(2);
    // 均分后每条车道的电线主干 = 2400 / 2 = 1200 ≤ 单条最高档。
    const perLane = wire.totalRate / plan.laneCount;
    expect(lanesForFlow(perLane)).toBe(1);
  });

  it('低吞吐（自动路线 5）→ laneCount=1（不拆车道）', () => {
